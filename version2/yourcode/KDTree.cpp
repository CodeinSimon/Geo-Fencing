...
inline Node* newNode(double arr[], int& id) {
    struct Node *temp = new Node;
    temp->point[0] = arr[0];
    temp->point[1] = arr[1];
    temp->min[0] = temp->max[0] = arr[0];
    temp->min[1] = temp->max[1] = arr[1];
...
inline void searchRangeRec(Node *(&root), double min[], double max[], int depth) {
    ...
    if (!root->isDeleted && max[0] > root->point[0] && max[1] > root->point[1]
        && min[0] < root->point[0] && min[1] < root->point[1]){
