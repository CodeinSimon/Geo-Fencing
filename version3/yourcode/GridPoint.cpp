...
void GridSetup(std::vector<std::pair<double, double> >& pgon, int numverts, int resolution, pGridSet p_gs) {
    std::pair<double, double> vtx0, vtx1, vtxa, vtxb;
    double *p_gl;
...
/* Set x variable for the direction of the ray */
        xdiff = vtxb.first - vtxa.first;
        ydiff = vtxb.second - vtxa.second;
...
/* if edge is of 0 length, ignore it (useless edge) */
        ...
        gcx = (int) ((vtxa.first - p_gs->minx) * p_gs->inv_xdelta);
        gcy = (int) ((vtxa.second - p_gs->miny) * p_gs->inv_ydelta);

/* get information about slopes of edge, etc */
        if (vtxa.first == vtxb.first) {
            sign_x = 0;
            tx = HUGE;
        } else {
            inv_x = tmax / xdiff;
            tx = p_gs->xdelta * (double) gcx + p_gs->minx - vtxa.first;
            if (vtxa.first < vtxb.first) {
                sign_x = 1;
                tx += p_gs->xdelta;
        ...
        if (vtxa.second == vtxb.second) {
            ty = HUGE;
        } else {
            inv_y = tmax / ydiff;
            ty = (p_gs->ydelta * (double) (gcy + 1) + p_gs->miny - vtxa.second)
                 * inv_y;
...
/* vertical line */
    ...
    if (p_gc->tot_edges <= 1) {
        ...
        p_gc->gr = (pGridRec) realloc(p_gc->gr, p_gc->tot_edges * sizeof(GridRec));
    }
...
int GridTest(pGridSet p_gs, double& x, double& y) {
    ...
    if ((ty = y) < p_gs->miny || ty >= p_gs->maxy
        || (tx = x) < p_gs->minx || tx >= p_gs->maxx) {
        /* outside of box */
        inside_flag = false;
    } else {
        /* what cell are we in? */
...
//    MALLOC_CHECK(p_gs->gc);
//
//    p_gs->minx = p_gs->maxx = pgon[0][X];
//    p_gs->miny = p_gs->maxy = pgon[0][Y];
//
///* find bounds of polygon */
//    for (i = 1; i < numverts; i++) {
//        vx0 = pgon[i][X];
